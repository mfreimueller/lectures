...
CREATE TABLE `games` (
    ...
    `release_year` INT,
    `genre` VARCHAR(50), -- MOBA, FPS, RTS, Fighting, etc.
    `active_status` TINYINT(1) DEFAULT 1, -- Is the game still supported?
    `description` TEXT
);
...
CREATE TABLE `teams` (
    ...
    `founded_year` INT,
    `country` VARCHAR(50) NOT NULL, -- Team's home country/region
...
CREATE TABLE `players` (
    ...
    `gamertag` VARCHAR(30) UNIQUE NOT NULL, -- In-game nickname
    `date_of_birth` DATE,
    `nationality` VARCHAR(50),
    `role` VARCHAR(30) NOT NULL, -- Carry, Support, Mid, Jungle, etc. (varies by game)
...
CREATE TABLE `tournaments` (
    ...
    `tournament_type` VARCHAR(30) NOT NULL, -- Championship, Qualifier, Invitational, Regular Season
    `format` VARCHAR(33), -- Single Elimination, Double Elimination, Round Robin
...
CREATE TABLE `tournament_teams` (
    ...
    `team_id` INT NOT NULL,
    `seed_number` INT, -- Tournament seeding rank
    `group_name` VARCHAR(20), -- Group stage identifier (Group A, B, C, etc.)
...
CREATE TABLE `matches` (
    ...
    `tournament_id` INT NOT NULL,
    `round_name` VARCHAR(50) NOT NULL, -- Group Stage, Quarterfinals, Semifinals, Finals, etc.
    `scheduled_datetime` DATETIME NOT NULL,
    `match_status` VARCHAR(30) DEFAULT 'upcoming',
...
CREATE TABLE `match_statistics` (
    ...
    `first_blood` TINYINT(1) DEFAULT 0, -- Did team get first kill?
...
CREATE TABLE `sponsors` (
    ...
    `industry_category` VARCHAR(50), -- Gaming Hardware, Energy Drinks, Tech, Automotive, etc.
...
CREATE TABLE `stream_viewership` (
    ...
    `stream_platform` VARCHAR(30) NOT NULL, -- Twitch, YouTube Gaming, Steam.tv, etc.
    `viewership_date` DATETIME DEFAULT NOW(),
