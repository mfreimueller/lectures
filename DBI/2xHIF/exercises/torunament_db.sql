...
CREATE TABLE "teams" (
    ...
    [founded_year] INT CHECK ([founded_year] >= 2005 AND [founded_year] <= YEAR(GETDATE())),
...
CREATE TABLE "tournaments" (
    ...
    [format] NVARCHAR(30), -- Single Elimination, Double Elimination, Round Robin
...
CREATE NONCLUSTERED INDEX [IX_sponsorships_active] ON sponsorships(contract_end_date DESC) WHERE contract_end_date >= CAST(GETDATE() AS DATE);

-- ============================================
-- VIEWS FOR COMMON QUERY PATTERNS (OPTIONAL TEACHING FEATURE)
-- ============================================
...
SELECT 
    ...
    SUM(CASE WHEN m.winner_team_id = tm.team_id THEN 1 ELSE 0 END) AS wins,
    SUM(CASE WHEN m.loser_team_id = tm.team_id THEN 1 ELSE 0 END) AS losses
FROM tournaments t
JOIN tournament_teams tm ON t.tournament_id = tm.tournament_id
...
GROUP BY t.tournament_id, tor.name, tm.team_id, te.team_name;

-- Active sponsorship value view
CREATE VIEW v_active_sponsorships AS
SELECT 
    s.sponsor_id,
    s.company_name,
    CASE 
        WHEN sp.tournament_id IS NOT NULL THEN 'Tournament'
        ELSE 'Team'
    END AS sponsor_type,
    COALESCE(t.name, te.team_name) AS sponsored_entity,
    sp.annual_value_usd,
    DATEDIFF(DAY, GETDATE(), sp.contract_end_date) AS days_remaining
FROM sponsors s
JOIN sponsorships sp ON s.sponsor_id = sp.sponsor_id
LEFT JOIN tournaments t ON sp.tournament_id = t.tournament_id
LEFT JOIN teams te ON sp.team_id = te.team_id
WHERE sp.contract_end_date >= CAST(GETDATE() AS DATE);

-- Top performing players view (aggregate stats across matches)
CREATE VIEW v_top_performing_players AS
SELECT 
    p.player_id,
    p.gamertag,
    ...
    CAST(SUM(ms.kills) AS FLOAT) / NULLIF(SUM(ms.deaths), 0) AS kda_ratio -- Kills+Deaths+Assists ratio simplified
FROM players p
JOIN team_memberships tm ON p.player_id = tm.player_id
WHERE tm.left_date IS NULL -- Only active players
LEFT JOIN tournament_teams tt ON tm.team_id = tt.team_id
LEFT JOIN matches m ON tt.tournament_id = m.tournament_id AND (m.winner_team_id = tm.team_id OR m.loser_team_id = tm.team_id)
LEFT JOIN match_statistics ms ON m.match_id = ms.match_id AND ms.team_id = tm.team_id
GROUP BY p.player_id, p.gamertag
HAVING COUNT(DISTINCT m.match_id) >= 5; -- Minimum matches threshold

-- ============================================
-- TRIGGERS FOR BUSINESS LOGIC (OPTIONAL TEACHING FEATURE)
-- ============================================

-- Trigger: Update team total wins when match is completed
CREATE TRIGGER trg_update_team_wins_on_match_completion
ON matches
AFTER INSERT, UPDATE
AS
BEGIN
    ...
    FROM teams t
    INNER JOIN inserted i ON t.team_id = i.winner_team_id
    WHERE i.match_status IN ('completed') AND i.winner_team_id IS NOT NULL;
    
    -- Reset win count for matches that were cancelled or changed status back to upcoming
    UPDATE teams
    SET total_wins = total_wins - 1
    FROM teams t
    INNER JOIN deleted d ON t.team_id = d.winner_team_id
    WHERE EXISTS (SELECT 1 FROM inserted i WHERE i.match_id = d.match_id AND i.status IN ('upcoming', 'cancelled'));
END;
GO

-- Trigger: Validate player age before insert/update
...
BEGIN
    ...
    SELECT TOP 1 @date_of_birth = date_of_birth FROM inserted;
    
    IF @date_of_birth IS NOT NULL
    BEGIN
        SET @age = DATEDIFF(YEAR, @date_of_birth, GETDATE());
        
        -- Check for age consistency (accounting for birthday in current year)
        IF DATEPART(DAYOFYEAR, GETDATE()) < DATEPART(DAYOFYEAR, @date_of_birth + CAST(@age AS INT))
            SET @age = @age - 1;
            
        IF @age < 16
        BEGIN
            RAISERROR('Player must be at least 16 years old to register for e-sports tournaments.', 16, 1);
            RETURN;
        END
    END
    
    -- Insert the actual data if validation passes
    INSERT INTO players (first_name, last_name, gamertag, date_of_birth, nationality, role, join_date)
    SELECT first_name, last_name, gamertag, date_of_birth, nationality, role, join_date FROM inserted;
END;
GO

-- ============================================
-- DEFAULT CONSTRAINTS FOR CONVENIENCE
-- ============================================

ALTER TABLE players ADD CONSTRAINT df_player_join_date DEFAULT GETDATE() FOR join_date;
ALTER TABLE stream_viewership ADD CONSTRAINT df_viewership_date DEFAULT GETDATE() FOR viewership_date;

-- Sample Data for E-Sports Tournament Management Platform
...
(3, 8, 1, NULL), -- NAVI top seed CS:GO Major
(3, 9, 2, 'Group A'), -- NiKo's team in Group A
...
(3, 'Group Stage Match A', '2024-03-17 15:00:00', 'completed', 8, 9), -- NAVI vs NiKo's team
(3, 'Semifinals', '2024-03-29 16:00:00', 'completed', NULL, 8); -- NAVI lost semifinal (winner TBD)
...
(1, 5, 18, 25, 43, NULL, 0), -- Cloud9 stats (lost match)
(2, 2, 22, 15, 55, NULL, 0), -- Fnatic won against FaZe Clan
(2, 6, 15, 22, 38, NULL, 1), -- FaZe Clan stats (got first blood but lost)
...
(6, 6, 38, 45, 76, 14, 0), 
(7, 8, 35, 30, 68, 22, 1), -- NAVI CS:GO Major performance
(7, 9, 30, 35, 55, 16, 0);
...
(2, 'Lower Bracket Round 1', '2023-10-20 14:00:00', 'completed', NULL, 1); -- Team Liquid lost
...
INSERT INTO stream_viewership ([match_id], [peak_concurrent_viewers], [average_viewers], [total_hours_watched]) VALUES 
(9, 3200000.00, 2400000.00, 78000000.00),
(10, 5100000.00, 3600000.00, 95000000.00);
...
(4, 8, '2024-01-01', '2027-12-31', 1200000.00); -- Intel long-term deal with NAVI
...
(9, 'Playoffs Semifinals', '2024-04-06 18:00:00', 'completed', NULL, 2); -- Fnatic advanced (winner TBD)
...
INSERT INTO stream_viewership ([match_id], [peak_concurrent_viewers], [average_viewers], [total_hours_watched]) VALUES 
(13, 1800000.00, 1300000.00, 35000000.00);
