...
Vergleichen Sie die beiden Creational Patterns **Singleton** und **Factory Method**. Implementieren Sie ein Mini-Beispiel, das beide Patterns kombiniert: Eine LoggerFactory, die als Singleton implementiert ist und verschiedene Logger-Typen (FileLogger, ConsoleLogger) per Factory Method erzeugt.
...
Fugen Sie einen dritten Logger-Typ (z.B. `DatabaseLogger`) hinzu, ohne die Factory-Klasse zu andern (Open/Closed Principle).
